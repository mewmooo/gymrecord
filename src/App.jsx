...
import { 
  ...
  PlayCircle, Save, Video, Zap, Skull, Scale
} from 'lucide-react';
  ...
  return (
    <div className="group bg-white dark:bg-[#0f1117] rounded-[28px] p-6 sm:p-7 shadow-sm hover:shadow-xl hover:shadow-violet-500/10 dark:shadow-none border border-gray-100 dark:border-gray-800/80 mb-6 transition-all duration-300 relative overflow-hidden">
      ...
      <div className="space-y-4 mb-6 empty:hidden">
        {showVideo && (
          <div className="rounded-[20px] overflow-hidden bg-[#0a0a0a] border border-gray-200 dark:border-gray-800 shadow-2xl relative aspect-video animate-in zoom-in-95 duration-300">
            {isAIVideoLoading ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900/90 backdrop-blur-md z-10">
                ...
                <p className="text-sm font-black text-white tracking-widest uppercase">AI Scanning YouTube...</p>
                <p className="text-xs text-gray-400 mt-2 font-medium">Mencari biomekanik tutorial terbaik.</p>
              </div>
            ) : exercise.videoId ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-50 dark:bg-[#11131a]">
                ...
                <p className="text-sm font-bold text-gray-500 dark:text-gray-400">Video tidak ditemukan</p>
              </div>
            )}
          </div>
        )}
              ...
              <div className="relative z-10">
                ...
                {aiBannerData.text ? (
                  <div className="bg-white/5 backdrop-blur-xl p-6 rounded-[20px] border border-white/10">
                    <p className={`text-[14px] leading-relaxed font-medium animate-in slide-in-from-bottom-2 ${aiBannerData.type === 'roast' ? 'text-rose-100' : 'text-violet-100'}`}>{aiBannerData.text}</p>
                  </div>
                ) : (
                  <p className="text-[13px] text-gray-400 font-medium max-w-[85%] leading-relaxed">Selesaikan set Anda, lalu minta AI memberikan evaluasi saintifik atau teguran motivasi tingkat tinggi.</p>
                )}
              </div>
            </div>
          )}
